...
function mapDomainErrorToStatus(err: any): { statusCode: number; code: string; message: string } {
  ...
  if (errMsg.includes('PACKAGE_INACTIVE') || errMsg.includes('INVALID_PACKAGE_CONFIGURATION')) {
    return { statusCode: 409, code: 'CONFLICT', message: 'الباقة المحددة غير صالحة أو غير نشطة' };
  }
  ...
  if (errMsg.includes('DELEGATE_DAILY_GARAGE_LIMIT_REACHED')) {
    return { statusCode: 409, code: 'CONFLICT', message: 'تم الوصول للحد الأقصى اليومي لإنشاء الجراجات' };
  }
