...
| Stage 9 — Shadow and compare | **In progress** | Preview-only normalized legacy/v2 providers for package catalog and date-scoped garage summary are wired into `POST /api/v2/shadow/compare` in `2c9186f`; comparisons are read-only, redacted, fail-closed, and disabled unless `V2_SHADOW_COMPARISON_ENABLED=true`. Local provider, route, emulator, build, and Production Gate validation passed. | Obtain a current Firebase-authenticated non-production Cloudflare preview, run real package and garage-summary comparisons, exercise v2-failure fallback and legacy-failure blocking, classify all differences, and prove no unexplained financial or authorization mismatch. No production shadow traffic is active. |
...
**Latest implementation commit:** `2c9186f feat: wire preview shadow read providers`, following the gated shadow comparison route in `07308e5`. The full emulator-backed v2 check passes locally with 55 test files and 293 tests; focused shadow tests add 9 passing tests; application tests, production build, maintainability checks, and diff checks also pass. Production Gate `35750806751` succeeded. Current Cloudflare Pages deployment evidence remains production-only; no current preview exists for authenticated frontend validation. Real providers are wired only into the guarded preview composition; no production shadow traffic is active and no authenticated live comparison is claimed.
