...
**Latest implementation commit:** `9c61429 feat: add fail-closed shadow read policy`, following the projection status endpoint in `6d2dc91`. The full emulator-backed v2 check passes locally with 52 test files and 280 tests, and all repair/status/shadow paths remain non-production or disabled by default. Current Cloudflare Pages deployment `6e04fbb4` is production-only; no current preview exists for authenticated frontend validation.
