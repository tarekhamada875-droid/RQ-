...
| Stage 4 — Authentication and policy enforcement | **Partial** | Session, per-device revocation, multi-device active-session markers, opaque active-session listing, inactivity, role, garage-scope, audit context, redaction, and rate-limit policies are unit-tested; injectable v2 Firebase token/session adapters, legacy-compatible per-device session records, authenticated `GET /api/auth/sessions` and `DELETE /api/auth/sessions/:sessionKey` controls, authorization middleware, CORS allowlisting, request-ID/context capture, UID-keyed rate limiting, structured console telemetry, explicit route budgets, and HTTP tests exist. The guarded preview is live on Railway and protected reads return `401` without authentication. | Complete an authenticated Cloudflare preview smoke test without changing production authority; add frontend UI wiring for session listing/revocation before broad rollout. |
...
| Stage 7 — Projections and reports | **Partial** | Projections, bounded read models, pending/activity Firestore repositories, cursor envelopes, emulator tests, daily financial rebuilds, reconciliation, report envelopes, repair-needed states, a production Firestore garage-summary adapter over legacy projection buckets/dashboard summaries, transactional Firestore projection persistence with event-id replay protection, a bounded deterministic rebuild primitive, an explicitly flagged admin-only authenticated repair route with idempotency/audit writes, an explicitly flagged read-only projection status route with missing/healthy/stale classification, an authenticated read-only dashboard report route exposing consistency, lag, differences, and repair-needed state, a bounded library-only projection-repair worker primitive with redacted failure classification, and a durable lease-owned Firestore repair-task queue with bounded retries exist. | Add a deployed production projection worker/scheduler and remaining real Firestore-backed reports; keep repair/status/report flags disabled until a controlled preview is authorized. |
...
| Stage 9 — Shadow and compare | **In progress** | Preview-only normalized legacy/v2 providers for package catalog and date-scoped garage summary are wired into `POST /api/v2/shadow/compare` in `2c9186f`; `01286d5` adds in-memory aggregate telemetry for outcomes, bounded latency, and Firestore read totals. Comparisons and telemetry are read-only, redacted, fail-closed, and disabled unless `V2_SHADOW_COMPARISON_ENABLED=true`. Focused local evidence now passes normalization, mismatch classification, equal-comparison permission, v2-failure fallback, legacy-failure blocking, rollback safety, route validation, and disabled-flag checks in 4 files/16 tests. | Obtain a current Firebase-authenticated non-production Cloudflare preview, run real package and garage-summary comparisons, exercise the same fallback/blocking checks through the preview, classify all differences, and prove no unexplained financial or authorization mismatch. No production shadow traffic is active. |
...
**Latest published commit:** `e67348c docs: record shadow telemetry coverage`, following shadow telemetry tests in `6fb49c6`, repair-lease safety in `54bd219`, projection-status coverage in `693fdb8`, the projection-repair runbook in `3b1f018`, calendar-date correction in `9b42c8f`, v2 adapter tests in `b1aae73`, Active Devices component tests in `eb19593`, service tests in `647e251`, Active Devices UI in `9bbcb5a`, typecheck correction in `b5b681b`, session-management backend in `5e2bba2`, `f0a5b20` CI propagation wait, the multi-device session work in `db0b4a5`, durable queue in `9896d3b`, worker in `8aee5e4`, dashboard report in `fa62c03`, aggregate telemetry in `01286d5`, provider wiring in `2c9186f`, and the gated shadow route in `07308e5`. The complete emulator-backed v2 check, full application suite, lint, production build, maintainability checks, and diff checks pass locally. Production Gate `35840571252` succeeded for `e67348c`; the succession handoff records that a dedicated non-production Cloudflare/Railway/Firebase environment is not being created in this slice. Active-session controls preserve legacy data and authorization boundaries; no financial authority or Cloudflare production flag changed. Cloudflare was rechecked on 2026-09-23 after `061204e`: latest deployment `fdf79720` is production for `main`/`061204e`, and every listed deployment is `environment: production`; no current non-production preview exists. Preview configuration has package-catalog enabled, but production is not preview evidence. The bounded local comparison/fallback/blocking evidence task and the follow-up service/component/adapter/date-contract/runbook/status-route/lease-safety/telemetry slices are complete; authenticated preview evidence remains blocked.
...
**Next implementation order:** run the immediately independent preparation tracks in parallel: (A) current authenticated Cloudflare preview validation whenever a natural non-production deployment and Firebase-authenticated session exist; (B) bounded non-financial repository, route, report, queue, and contract hardening; (C) projection-worker operational design, observability, non-production harness, and rollback preparation without deployment or automatic mutation; and (D) local migration-evidence, comparison, and rollback documentation. Track A remains blocked until a legitimate current preview exists; do not manufacture one or change production flags. After the parallel preparation tracks complete, serialize the dependent gates: (1) live normalized package and garage-summary comparison through the preview-only provider; (2) preview fallback/blocking evidence and mismatch classification; (3) SLO, cost, reconciliation, and rollback review; (4) progressive read cutover from internal users through cohorts; (5) financial authority migration only after transaction, idempotency, reconciliation, and rollback gates; and (6) legacy retirement only after migration completion and rollback-window expiration.

### Parallel execution model

The remaining preparation work is intentionally divided into independent workstreams. Each workstream must remain bounded, preserve legacy authority, and publish only after its own focused tests plus the full repository validation gate pass.

| Workstream | Scope that may proceed now | Blocking dependency | Explicit prohibition |
| --- | --- | --- | --- |
| **A — Authenticated preview validation** | Inspect natural Cloudflare Pages previews; when a current non-production preview exists, run Firebase-authenticated health, package, garage-summary, and session checks, then perform normalized comparison and fallback/blocking exercises. | Current non-production preview URL and Firebase-authenticated browser session. | Do not create a branch to manufacture a preview, use stale or production deployments as evidence, alter production flags, or use the Railway operator token as a Firebase user token. |
| **B — Non-financial hardening** | Add or correct contracts, repository idempotency/authorization tests, route boundary tests, report consistency tests, queue lease/retry tests, pagination tests, and error-redaction coverage. | Existing legacy behavior and matching v2 contracts; no live preview required. | Do not migrate financial writes, delete production data, remove legacy routes, or broaden runtime authority merely to improve coverage. |
| **C — Projection-worker operations** | Define configuration, health/failure metrics, alert thresholds, non-production invocation harness, queue lease/retry behavior, flag-off path, evidence capture, and rollback rehearsal. | Existing worker, queue, and runbook contracts; deployment requires a separate operational review. | Do not deploy, schedule, or automatically trigger a production mutator until non-production evidence, alerts, flag-off behavior, and rollback rehearsal are complete. |
| **D — Migration evidence and rollback preparation** | Extend pure normalization, mismatch classification, redacted telemetry, fallback/blocking tests, and migration-safety documentation. | Existing providers and test fixtures; live preview is required only for external evidence. | Do not enable production shadow traffic, dual financial writes, or cutover based solely on local evidence. |

The workstreams may be developed concurrently, but their commits remain independently validated and reviewable. The following stages are **not parallelizable** because each depends on the preceding evidence: preview comparison → fallback/blocking proof → SLO/cost/reconciliation/rollback review → progressive read cohorts → financial authority migration → legacy retirement.
