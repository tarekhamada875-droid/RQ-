...
**Latest implementation commit:** `5a0cb56 feat: add shadow comparison coordinator`, following the fail-closed policy in `9c61429`. The full emulator-backed v2 check passes locally with 53 test files and 284 tests, and all repair/status/shadow paths remain non-production or disabled by default. Current Cloudflare Pages deployment `26c2151f` is production-only; no current preview exists for authenticated frontend validation.
