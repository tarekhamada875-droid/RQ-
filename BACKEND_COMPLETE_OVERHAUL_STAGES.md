...
**Latest published commit:** `54bd219 fix: reject expired repair leases`, following projection-status coverage in `693fdb8`, the projection-repair runbook in `3b1f018`, calendar-date correction in `9b42c8f`, v2 adapter tests in `b1aae73`, Active Devices component tests in `eb19593`, service tests in `647e251`, Active Devices UI in `9bbcb5a`, typecheck correction in `b5b681b`, session-management backend in `5e2bba2`, `f0a5b20` CI propagation wait, the multi-device session work in `db0b4a5`, durable queue in `9896d3b`, worker in `8aee5e4`, dashboard report in `fa62c03`, aggregate telemetry in `01286d5`, provider wiring in `2c9186f`, and the gated shadow route in `07308e5`. The complete emulator-backed v2 check, full application suite, lint, production build, maintainability checks, and diff checks pass locally. Production Gate `35837731215` succeeded for `54bd219`; it makes repair-queue completion/failure reject expired leases without changing queue state, with emulator coverage and runbook evidence. Active-session controls preserve legacy data and authorization boundaries; no financial authority or Cloudflare production flag changed. Cloudflare was rechecked on 2026-09-23 after `061204e`: latest deployment `fdf79720` is production for `main`/`061204e`, and every listed deployment is `environment: production`; no current non-production preview exists. Preview configuration has package-catalog enabled, but production is not preview evidence. The bounded local comparison/fallback/blocking evidence task and the follow-up service/component/adapter/date-contract/runbook/status-route/lease-safety slices are complete; authenticated preview evidence remains blocked.
