...
| Stage 7 — Projections and reports | **Partial** | Projections, bounded read models, pending/activity Firestore repositories, cursor envelopes, emulator tests, daily financial rebuilds, reconciliation, report envelopes, repair-needed states, a production Firestore garage-summary adapter over legacy projection buckets/dashboard summaries, transactional Firestore projection persistence with event-id replay protection, a bounded deterministic rebuild primitive, an explicitly flagged admin-only authenticated repair route with idempotency/audit writes, an explicitly flagged read-only projection status route with missing/healthy/stale classification, an authenticated read-only dashboard report route exposing consistency, lag, differences, and repair-needed state, and a bounded library-only projection-repair worker primitive with redacted failure classification exist. | Add a deployed production projection worker/scheduler and remaining real Firestore-backed reports; keep repair/status/report flags disabled until a controlled preview is authorized. |
...
**Latest implementation commit:** `8aee5e4 feat: add bounded projection repair worker`, following the dashboard report in `fa62c03`, aggregate telemetry in `01286d5`, provider wiring in `2c9186f`, and the gated shadow route in `07308e5`. The full emulator-backed v2 check passes locally with 55 test files and 293 tests; focused worker/repair tests pass with 12 tests, prior provider/telemetry/route/report tests pass with 17 tests, full application tests pass with 55 files and 296 tests, and production build, maintainability, and diff checks also pass. Production Gate `35757199426` succeeded. Cloudflare Pages inspection at 2026-09-22 19:09 UTC+3 found no current preview for `main`; the newest preview is the stale 2026-09-19 `feat/backend-operator-mcp-auth` deployment. The worker is library-only; no automatic production repair or shadow traffic is active, and no authenticated live comparison is claimed.
