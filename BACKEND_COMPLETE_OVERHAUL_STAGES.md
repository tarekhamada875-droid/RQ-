...
| Stage 4 — Authentication and policy enforcement | **Partial** | Session, revocation, inactivity, role, garage-scope, audit context, redaction, and rate-limit policies are unit-tested; injectable v2 Firebase token/session adapters, authorization middleware, CORS allowlisting, and HTTP tests now exist. | Add request-context logging/rate limiting and wire the guarded auth dependencies into a Railway preview without changing production authority. |
...
**Next implementation order:** (1) request-context logging and rate limiting; (2) guarded Railway preview deployment under `/api/v2` with auth dependencies; (3) one Cloudflare preview read feature; (4) authenticated end-to-end smoke tests; (5) transactional lifecycle commands; (6) shadow comparison; (7) progressive cutover; (8) legacy retirement.
