...
| Stage 7 — Projections and reports | **Partial** | Projections, bounded read models, pending/activity Firestore repositories, cursor envelopes, emulator tests, daily financial rebuilds, reconciliation, report envelopes, repair-needed states, a production Firestore garage-summary adapter over legacy projection buckets/dashboard summaries, transactional Firestore projection persistence with event-id replay protection, and a bounded deterministic projection rebuild primitive exist. | Add production projection workers, authenticated repair/rebuild operations, lag telemetry, and remaining real Firestore-backed reports. |
...
**Latest implementation commit:** `5825c4b feat: add bounded projection rebuild primitive`, following the transactional persistence slice in `9276bd1`. The full emulator-backed v2 check passes locally with 49 test files and 268 tests. Do not infer the new Production Gate result until it completes.
