...
### 2026-09-25 — vehicle checkout rollback dependency correction

- Reviewed the checkout callback’s optimistic removal, server-authoritative checkout request, cost reconciliation, and rollback paths. Confirmed `setGarage` and `setVehicles` are stable `useState` dispatchers supplied by `useGarageApp`; `vehicles` is read to snapshot the current list before optimistic removal. Added exactly those three dependencies, preventing stale rollback closures without changing listener ownership, API behavior, or UI semantics.
