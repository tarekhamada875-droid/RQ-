...
const result = {
  operationId: 'subscriber_renew_1',
  subscriber: {
    id: 'plate_YWJjLTEyMw', garageId: 'garage-1', plate: 'ABC123', status: 'active' as const,
    startAt: '2026-10-21T00:00:00.000Z', endAt: '2026-11-21T00:00:00.000Z', updatedAt: '2026-10-21T12:00:00.000Z'
  }
};

const body = {
  startAt: '2026-10-21T00:00:00.000Z', endAt: '2026-11-21T00:00:00.000Z', idempotencyKey: 'renewal-0001'
};
...
describe('v2 subscriber-renew route', () => {
  it('validates and forwards an authenticated renewal', async () => {
    const { baseUrl, calls } = await start();
    const response = await fetch(`${baseUrl}/v2/garages/garage-1/subscribers/plate_YWJjLTEyMw/renew`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  ...
  it('rejects malformed input before calling the repository', async () => {
    const { baseUrl, calls } = await start();
    const response = await fetch(`${baseUrl}/v2/garages/garage-1/subscribers/plate_YWJjLTEyMw/renew`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...body, idempotencyKey: 'short' }) });
  ...
  it('rejects cross-garage access before calling the repository', async () => {
    const { baseUrl, calls } = await start();
    const response = await fetch(`${baseUrl}/v2/garages/garage-2/subscribers/plate_YWJjLTEyMw/renew`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  ...
  it('maps renewal domain and idempotency conflicts to 409', async () => {
    const subscriberCommands: SubscriberCommandRepository = { create: async () => { throw new Error('unused'); }, renew: async () => { throw new Error('SUBSCRIBER_CANCELLED'); }, update: async () => { throw new Error('unused'); }, suspend: async () => { throw new Error('unused'); }, cancel: async () => { throw new Error('unused'); }, delete: async () => { throw new Error('unused'); } };
    ...
    await new Promise<void>((resolve) => server?.once('listening', () => resolve()));
    const address = server?.address() as AddressInfo;
    const response = await fetch(`http://127.0.0.1:${address.port}/v2/garages/garage-1/subscribers/plate_YWJjLTEyMw/renew`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    expect(response.status).toBe(409);
  });

  it('does not expose the write route without the authenticated preview gate', async () => {
    const app = createV2App({ environment: parseEnvironment({ NODE_ENV: 'production', FIREBASE_PROJECT_ID: 'rq-v2-route-test' }), subscriberCommands: { create: async () => { throw new Error('unused'); }, renew: async () => result, update: async () => { throw new Error('unused'); }, suspend: async () => { throw new Error('unused'); }, cancel: async () => { throw new Error('unused'); }, delete: async () => { throw new Error('unused'); } } });
    server = app.listen(0);
    await new Promise<void>((resolve) => server?.once('listening', () => resolve()));
    const address = server?.address() as AddressInfo;
    const response = await fetch(`http://127.0.0.1:${address.port}/v2/garages/garage-1/subscribers/plate_YWJjLTEyMw/renew`, { method: 'POST' });
