...
describe('v2 subscriber-create route', () => {
  ...
  it('maps duplicate and idempotency conflicts to 409', async () => {
    const repository: SubscriberCommandRepository = { create: async () => { throw new Error('SUBSCRIBER_ALREADY_EXISTS'); }, renew: async () => { throw new Error('unused'); }, update: async () => { throw new Error('unused'); }, suspend: async () => { throw new Error('unused'); } };
  ...
  it('does not expose the write route without the authenticated preview gate', async () => {
    const app = createV2App({ environment: parseEnvironment({ NODE_ENV: 'production', FIREBASE_PROJECT_ID: 'rq-v2-route-test' }), subscriberCommands: { create: async () => result, renew: async () => { throw new Error('unused'); }, update: async () => { throw new Error('unused'); }, suspend: async () => { throw new Error('unused'); } } });
    server = app.listen(0);
    await new Promise<void>((resolve) => server?.once('listening', () => resolve()));
    const address = server?.address() as AddressInfo;
